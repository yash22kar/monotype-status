...
def tab_daily(cdf: pd.DataFrame, mdf: pd.DataFrame):
    ...
    with h2:
        sel_date = st.date_input("", value=date.today(), label_visibility="collapsed")
        st.markdown(
    ...
    n_qa   = int(done_today["qa_status"].sum())  if not done_today.empty else 0
    n_fud  = int(done_today["fud_status"].sum()) if not done_today.empty else 0
    ...
    for r in RESEARCHERS:
        ...
        summary_rows.append({
            ...
            "QA ✅ Today":  int(r_done_today["qa_status"].sum())  if not r_done_today.empty else 0,
            "FUD ✅ Today": int(r_done_today["fud_status"].sum()) if not r_done_today.empty else 0,
        })
    st.dataframe(
    ...
    for researcher in RESEARCHERS:
        ...
        qa_today   = int(r_done_today["qa_status"].sum())  if not r_done_today.empty else 0
        fud_today  = int(r_done_today["fud_status"].sum()) if not r_done_today.empty else 0
            ...
            with st.container():
                ...
                if not r_done_today.empty:
                    ...
                    with st.form(f"unmark_{researcher}_{date_str}"):
                        ...
                        st.markdown("")
                        if st.form_submit_button("↩️ Unmark Selected (revert to Pending)", use_container_width=True):
                            if to_unmark:
...
def tab_companies(cdf: pd.DataFrame):
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "FUD": st.column_config.TextColumn(width="small"),
            "Date Completed": st.column_config.TextColumn(width="medium"),
        },
        disabled=["ID", "Company Name", "Status", "Subsidiaries", "QA", "FUD", "Date Completed"],
        hide_index=True,
        use_container_width=True,
        key=editor_key,
    )

    # ── Detect assignee changes and offer a save button ──
    changed_rows = edited[
        edited.apply(lambda r: r["Assigned To"] != original_assignees.get(r["ID"], "— Unassigned —"), axis=1)
    ]
    if not changed_rows.empty:
        st.info(f"**{len(changed_rows)} assignee change(s) pending** — click Save to apply.")
        if st.button("💾 Save Assignee Changes", type="primary", key="btn_save_assign"):
            ...
            bust_cache()
            st.session_state.op_count += 1
            st.success(f"Saved assignee changes for {len(changed_rows)} company/companies.")
            st.rerun()
